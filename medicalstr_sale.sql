...
CREATE TABLE `sale` (
  ...
  KEY `item_name_idx` (`m_name`),
  KEY `item_price_idx` (`item_price`),
  CONSTRAINT `item_name` FOREIGN KEY (`m_name`) REFERENCES `medicines` (`m_name`) ON DELETE CASCADE ON UPDATE CASCADE
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `sale_BEFORE_INSERT` BEFORE INSERT ON `sale` FOR EACH ROW BEGIN
     update medicines set stock=(stock- new.quantity) where m_name= new.m_name;
END */;;
DELIMITER ;
...
-- Dump completed on 2023-07-16  8:58:30
